...
def getUsername():
    if rndd == "true":
        ...
        letters = random.choices(string.ascii_uppercase, k=10)

        sample = random.sample(digits + letters, 18)
...
"""))
    ...
    for i in range(countz):
        ...
        try:
            ...
            WebDriverWait(driver, 2).until(EC.presence_of_element_located((By.XPATH, '/html/body/div/div/div[1]/button')))
            try:
                capBtn = driver.find_element(By.XPATH, "/html/body/div/div/div[1]/button")
                if capBtn.is_displayed == True:
                    print("Button Found")
                    if capBtn.text.lower() == "start puzzle":
                        #driver.find_element(By.XPATH, '/html/body/div/div/div[1]/button')
                        print("No crash")
                        time.sleep(1)
                        driver.quit()
                        print("Bad Captcha")
                    else:
                        print("The text does not equal 'start puzzle'")
                else:
                    print("I aint find no button")
...
def starter():
    ...
    if rnd_usernam == "y" or rnd_usernam == "Y":
        ...
        clear()
        starter()
    key0 = input("Enter your captcha key (N if none): ")
    time.sleep(0.5)
